...
void perform_traceroute(const char *host, uint32_t max_hops, uint32_t timeout_ms)
{
    ...
    // so a trace route works by sending icmp echo requests with increasing ttl values, starting from 1 up to max_hops
    // this allows us to discover each hop along the route to the destination
    for (uint32_t hop = 1; hop <= max_hops; hop++) 
    {
        ...
        // for each hop we will send 3 probes (tests) and wait for replies
        for (int probe = 0; probe < 3; probe++)
        {
            ...
            // printf("Receive icmp reply returned %d\n", r);
